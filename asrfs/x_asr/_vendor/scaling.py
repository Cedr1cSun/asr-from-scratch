...
# VENDOR surgery: torch>=2.4 moved custom_fwd/custom_bwd to torch.amp and made
# `device_type` a required keyword. Bind it via functools.partial so the existing
# bare @custom_fwd / @custom_bwd decorator sites keep working unchanged.
...
class SwooshL(torch.nn.Module):
    def forward(self, x: Tensor) -> Tensor:
        ...
        if not x.requires_grad:
            # VENDOR surgery: k2.swoosh_l_forward -> in-file pure-torch SwooshLForward
            return SwooshLForward(x).to(x.dtype)
        else:
            # VENDOR surgery: k2.swoosh_l -> in-file autograd SwooshLFunction
            return SwooshLFunction.apply(x).to(x.dtype)
...
class SwooshR(torch.nn.Module):
    def forward(self, x: Tensor) -> Tensor:
        ...
        if not x.requires_grad:
            # VENDOR surgery: k2.swoosh_r_forward -> in-file pure-torch SwooshRForward
            return SwooshRForward(x).to(x.dtype)
        else:
            # VENDOR surgery: k2.swoosh_r -> in-file autograd SwooshRFunction
            return SwooshRFunction.apply(x).to(x.dtype)
...
# VENDOR surgery: pure-torch replacements for k2.swoosh_{l,r}_forward_and_deriv,
# used by ActivationDropoutAndLinearFunction's custom backward after the top-level
# k2 dependency was removed. Each returns (activation(x), d activation / d x); the
# derivative is
# the exact analytic gradient of the corresponding Swoosh defined above:
#   d/dx swoosh_l(x) = sigmoid(x - 4.0) - 0.08
#   d/dx swoosh_r(x) = sigmoid(x - 1.0) - 0.08
def SwooshLForwardAndDeriv(x: Tensor):
    return SwooshLForward(x), (torch.sigmoid(x - 4.0) - 0.08).to(x.dtype)


def SwooshRForwardAndDeriv(x: Tensor):
    return SwooshRForward(x), (torch.sigmoid(x - 1.0) - 0.08).to(x.dtype)
...
class ActivationDropoutAndLinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        forward_activation_dict = {
            # VENDOR surgery: k2.swoosh_{l,r}_forward -> in-file pure-torch versions
    ...
    def backward(ctx, ans_grad: Tensor):
        ...
        forward_and_deriv_activation_dict = {
            # VENDOR surgery: k2.swoosh_{l,r}_forward_and_deriv -> in-file pure-torch versions
