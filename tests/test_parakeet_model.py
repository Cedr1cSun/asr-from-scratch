...
def test_small_random_init():
    ...
    assert 20e6 < report["params_total"] < 40e6  # 目标 ~26M
    assert report["params_total"] == report["params_trainable"] or report["frozen"], "frozen set must be explicit"
    assert model.config.pad_token_id == model.config.vocab_size - 1  # blank last


def test_tokenizer_vocab_alignment():
    tok = build_tokenizer()
    model = build_model()
    assert model.config.vocab_size == tok.vocab_size + 1  # +1 = CTC blank
